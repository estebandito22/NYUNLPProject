"""Module for BLEU score evaluation."""
...
class BleuEvaluator(object):
    ...
    """Class to compute BLEU score based on sacrebleu."""

    def __init__(self):
        """Initialize BleuEvaluator."""
        self.NGRAM_ORDER = 4
        self.BLEU = namedtuple(
            'BLEU', 'score, counts, totals, precisions, bp, sys_len, ref_len')
    ...
    def _extract_ngrams(self, line, min_order=1):
        """
        Extract all the ngrams (1 <= n <= NGRAM_ORDER) from a token sequence.
    ...
    def _compute_bleu(self, correct, total, sys_len, ref_len, smooth='none',
                      smooth_floor=0.01, use_effective_order=False):
        """
        Compute BLEU score from its sufficient statistics. Adds smoothing.
        ...
        if sys_len < ref_len:
            brevity_penalty = math.exp(1 - ref_len / sys_len) \
                if sys_len > 0 else 0.0

        bleu = brevity_penalty * math.exp(
            sum(map(self._my_log,
                    precisions[:effective_order])) / effective_order)

        return self.BLEU._make(
            [bleu, correct, total, precisions, brevity_penalty,
             sys_len, ref_len])

    def corpus_bleu(self, sys_stream, ref_streams, smooth='exp',
                    smooth_floor=0.0, force=False, lowercase=False,
                    tokenize='none', use_effective_order=False):
        """
        Produce BLEU scores along with its sufficient statistics from a
        ...
        for lines in zip_longest(*fhs):
            ...
            output, *refs = [
                self.TOKENIZERS[tokenize](x.rstrip()) for x in lines]

            ref_ngrams, closest_diff, closest_len = \
                self._ref_stats(output, refs)
        ...
        return self._compute_bleu(correct, total, sys_len, ref_len, smooth,
